...
def main(N=5, dim=2, dt=1.0e-3,stride=100, useN=None,grid=False, withdata=None, datapoints=100, ifdrag=0, seed=100, rname=0, saveovito=1, trainm=0, runs=200, semilog=1, maxtraj=100, plotthings=True, redo=0):
    if useN is None:
        useN = N
    ...
    zdot, lamda_force = get_zdot_lambda(
        N, dim, hamiltonian=Hactual, drag=None, constraints=None, external_force=None)
    ...
    params = loadfile(f"hnn_trained_model.dil", trained=useN)[0]
    ...
    for ind in range(maxtraj):
        ...
        if plotthings:
            if ind < 1:
                ...
                title = f"HNN {N}-Spring Exp {ind} Hmodel"
                axs[1].set_title(title)
                title = f"HNN {N}-Spring Exp {ind} Hactual"
