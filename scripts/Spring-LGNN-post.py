...
def main(N=3, dt=1.0e-5, useN=None, withdata=None, datapoints=100, mpass=1, grid=False, stride=100, ifdrag=0, seed=42, rname=0, saveovito=1, trainm=0, runs=1000, semilog=1, maxtraj=1, plotthings=True, redo=0):
    ...
    if trainm:
        ...
        def L_energy_fn(params, graph):
            g, V, T = cal_graph(params, graph, mpass=mpass, eorder=eorder,
                                useT=True, useonlyedge=True)
            print('hi')
            print(kin_energy(graph.nodes["velocity"]), V)
