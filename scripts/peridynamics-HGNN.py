...
volume = []
import pandas as pd
for num in (np.linspace(1,1000,51).astype('int')):
    dataf_name = f"env_1_step_{num}.jld.data"
    df = pd.read_csv(f'../results/peridynamics-data/test1/{dataf_name}')    
...
my_graph0_disc = util.make_graph(o_position,displacement,atoms={0: 125},cutoff=3.0)
