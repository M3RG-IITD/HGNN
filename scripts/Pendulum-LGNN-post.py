...
def main(N=2, dim=2, dt=1.0e-5, useN=None, stride=1000, ifdrag=0, seed=42, rname=0,  saveovito=1, trainm=0, runs=100, semilog=1, maxtraj=100, plotthings=True, redo=0):
    if useN is None:
        useN = N
    print("Configs: ")
        ...
        if plotthings:
            if ind<1:
                for key, traj in {"actual": actual_traj, "pred": pred_traj}.items():
                    ...
                    plt.savefig(_filename(f"net_force_Exp_{ind}_{key}_{ifdrag}.png"))
