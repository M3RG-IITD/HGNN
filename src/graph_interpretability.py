...
def GNNet(
    ...
    def _ApplyGraphNet(graph):
        ...
        return gn_graph.GraphsTuple(
            ...
            globals=globals_,
            n_node=n_node,
            n_edge=n_edge), Vij, Vi, Ti, edges['eij']
...
def cal_graph(params, graph, eorder=None, mpass=1,
    ...
    def initial_edge_emb_fn(edges, senders, receivers, globals_):
        ...
        # eij = dr
        eij = jnp.sqrt(1e-10 + jnp.square(dr).sum(axis=1, keepdims=True))
        # print(eij.shape)
        emb = fb(eij)
    ...
    if useonlyedge:
        def edge_node_to_V_fn(edges, nodes):
            ...
            # print(vij, edges["eij"])
            return vij.flatten(), jnp.array(0.0)
    else:
        def edge_node_to_V_fn(edges, nodes):
            vij = ff1(edges["edge_embed"]).flatten()
            vi = 0
            vi = vi + ff2(nodes["node_embed"]).flatten()
            vi = vi + ff3(nodes["node_pos_embed"]).flatten()
            return vij, vi
    
    def node_to_T_fn(nodes):
        return ke(nodes["node_vel_embed"]).flatten()
