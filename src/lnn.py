...
def accelerationFull_massL(n, Dim, lagrangian=lagrangian, non_conservative_forces=None, external_force=None, constraints=None):
    """ ̈q = M⁻¹(-C ̇q + Π + Υ - Aᵀ(AM⁻¹Aᵀ)⁻¹ ( AM⁻¹ (-C ̇q + Π + Υ + F ) + Adot ̇qdot ) + F )
    ...
    def fn(x, v, params):
        ...
        # M = d2L_dv2(x.flatten(), v.flatten(), params)
        # 2
        _mass = forward_pass(params["M_1"], v.reshape(-1, ))
...
def acceleration_GNODE(n, Dim, F_q_qdot, constraints=None):
