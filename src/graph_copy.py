...
def cal_acceleration(params, graph, eorder=None, mpass=1,
              useT=True, useonlyedge=False, act_fn=SquarePlus):
    fb_params = params["fb"]
    fne_params = params["fne"]
    fneke_params = params["fneke"]
    ...
    def edge_node_to_acceleration(edges, nodes):
        # vij = ff1(edges["edge_embed"]).sum()
        vi = 0
        vi = vi + ff2(nodes["node_embed"])
        vi = vi + ff3(nodes["node_pos_vel_embed"])
        return vi
    ...
    Net = GNNet(N=mpass,
                ...
                update_node_fn=update_node_fn)
    graph, V, T = Net(graph)
    return edge_node_to_acceleration(graph.edges, graph.nodes)
